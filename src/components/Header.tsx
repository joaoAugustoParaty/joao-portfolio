...
export function Header({ language, theme, text, onLanguage, onTheme }: HeaderProps) {
  ...
  return <header className="site-header">
    ...
    <nav id="main-navigation" className={open ? 'main-nav is-open' : 'main-nav'} aria-label={text.navLabel}>
      ...
      <a className="nav-cta" href={getResumeHref(language)} target="_blank" rel="noreferrer">{text.resume}<span className="sr-only"> — {language === 'pt' ? 'abre em nova aba' : 'opens in a new tab'}</span> <ArrowUpRight size={15} /></a>
    </nav>
  </header>
}
