  ...
  return <section id="contato" className="contact section-block">
    <div className="section-shell">
      ...
      <div className="contact-layout">
        <div className="contact-info reveal"><div className="contact-glyph" aria-hidden="true">✦</div><p>{text.direct}</p><a href={`mailto:${emailAddress}`}><Mail /> {emailAddress}</a><button className="copy-email" type="button" onClick={copyEmail}>{copyStatus === 'success' ? <Check /> : <Copy />}{copyStatus === 'success' ? text.copiedEmail : copyStatus === 'error' ? text.copyError : text.copyEmail}</button><span className="sr-only" aria-live="polite">{copyStatus === 'success' ? text.copiedEmail : copyStatus === 'error' ? text.copyError : ''}</span><div className="socials"><a href="https://github.com/joaoaugustodeoliveiraneto" target="_blank" rel="noreferrer"><Code2 /> GitHub<span className="sr-only"> — {text.newTab}</span></a><a href="https://www.linkedin.com/in/joao-augusto-de-oliveira-neto/" target="_blank" rel="noreferrer"><BriefcaseBusiness /> LinkedIn<span className="sr-only"> — {text.newTab}</span></a></div></div>
